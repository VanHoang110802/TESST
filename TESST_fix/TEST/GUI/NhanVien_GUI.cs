...
namespace TEST.GUI
{
    public partial class NhanVien_GUI : Form
    {
        ...
        private void button_Sua_Click(object sender, EventArgs e)
        {
            ...
            // Lấy DataRowView từ dòng được chọn
            var dataRowView = (DataRowView)dataGridView_ThongTin.SelectedRows[0].DataBoundItem;

            // Chuyển đổi DataRowView thành NhanVien_DTO
            NhanVien_DTO nv = new NhanVien_DTO
            {
                MaNV = Convert.ToInt32(dataRowView["MaNV"]),
                HoTen = dataRowView["HoTen"].ToString(),
                NgaySinh = Convert.ToDateTime(dataRowView["NgaySinh"]),
                GioiTinh = dataRowView["GioiTinh"].ToString(),
                Luong = Convert.ToDecimal(dataRowView["Luong"])
            };

            // Mở form sửa
            Sua_GUI suaForm = new Sua_GUI(nv);
